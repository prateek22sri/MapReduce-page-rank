...
import indiana.cgl.hadoop.pagerank.helper.SortingMap;
import indiana.cgl.hadoop.pagerank.helper.SortingReduce;
...
public class HadoopPageRank extends Configured implements Tool {
	...
	public static void main(String[] args) throws Exception {
		...
		FileOutputFormat.setOutputPath(job1,
				new Path(String.valueOf(outputIndex)));
		...
		// ///////////////////////////////////////////////////
		// core computation that require multiple iterations to make rank values
		// results converge
		for (int i = 0; i < noIterations; i++) {
			...
			// the output in the current iteration will become input in next
			// iteration.
			FileInputFormat.setInputPaths(job2,
					new Path(String.valueOf(outputIndex)));
			FileOutputFormat.setOutputPath(job2,
					new Path(String.valueOf(outputIndex + 1)));
			...
			if (!job2.isSuccessful()) {
				System.out.format("Hadoop PageRank iteration:{" + i
						+ "} failed, exit...", i);
				System.exit(-1);
			}
		...
		FileInputFormat.setInputPaths(job3,
				new Path(String.valueOf(outputIndex)));
		...
		System.out.println("Hadoop Sorting starts...\n");
		// ///////////////////////////////////////////////////
		// collect the (url_id,rank_value) tuple from output in stage 3

		Job job4 = new Job(config, "Sorting");
		...
		FileInputFormat.setInputPaths(job4,
				new Path(String.valueOf(outputIndex)));
		FileOutputFormat.setOutputPath(job4,
				new Path(String.valueOf(outputDir)));
		...
		System.out
				.println("########################################################");
		System.out.println("#   Hadoop PageRank Job take " + executionTime
				+ " sec.");
		System.out
				.println("########################################################");
		System.exit(0);
